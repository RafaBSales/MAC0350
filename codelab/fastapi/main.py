...
async def get_users(index: int | None = None):
    ...
    if index == None:
        return {"status": "success", "user": [{"nome": user.nome, "idade": user.idade} for user in users]}

    try:
        user = users[index]
    except IndexError:
        raise HTTPException(status_code=500, detail=f"Lista possui apenas {len(users)} usuário(s)")
    
    return {"status": "success", "user": {"nome": user.nome, "idade": user.idade}}
